...
export default function NotebookCell({ nodeId }: { nodeId: string }) {
  ...
  const hasOutput = Boolean(node?.lastOutput);
  const hasError = Boolean(node?.lastError);
  const stdoutText = node?.lastOutput ?? "Run this cell to see output here.";
  const stderrText = node?.lastError ?? "No error output.";
      ...
      state: EditorState.create({
        ...
        extensions: [
          ...
          EditorView.updateListener.of((update) => {
            if (!update.docChanged || suppressSyncRef.current) {
              return;
            }
  ...
  }, [node, nodeId, setActiveNodeId, updateNode]);
  ...
  return (
    <motion.article className={`code-cell code-cell--${node.status}`} layout>
      ...
      <section className="code-cell__console" aria-live="polite">
        <div className={`code-cell__output ${hasOutput ? "code-cell__output--stdout" : "code-cell__output--empty"}`}>
          <span className="code-cell__output-label">stdout</span>
          <pre className="code-cell__output-text">{stdoutText}</pre>
        </div>
        <div className={`code-cell__output ${hasError ? "code-cell__output--error" : "code-cell__output--empty"}`}>
          <span className="code-cell__output-label">stderr</span>
          <pre className="code-cell__output-text">{stderrText}</pre>
        </div>
      </section>
    </motion.article>
  );
